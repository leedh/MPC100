...
def register2MNI_ITK(fslDir, inFile, outFile, ITKcoreg, ITKaffine, ITKwarp):
    """ This function registers an image (or time-series of images) to MNI152 T1 2mm using ITK format transformation files.
        ...
        inFile:     Full path to the data file (nii.gz) which has to be registerd to MNI152 T1 2mm
        outFile:    Full path of the output file
        ITKcoreg:   Full path of the mat file describing the affine registration of the functional data to structural space
        ITKaffine:  Full path of the mat file describing the affine registration of the structural data to MNI152 space
        ITKwarp:    Full path of the warp file describing the non-linear registration of the structural data to MNI152 space
    ...
    fslnobin = fslDir.rsplit('/', 2)[0]
    ref = os.path.join(fslnobin, 'data', 'standard', 'MNI152_T1_2mm_brain.nii.gz')
    os.system(' '.join(['antsApplyTransforms',
                        '--verbose 1',
                        '--dimensionality 3',
                        '--input-image-type 3',
                        '--float 1',
